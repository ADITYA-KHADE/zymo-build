...
const Cars = () => {
  ...
  return (
    <div className="relative">
      ...
      <div
        className="relative w-full h-[500px]" // Height set to 100vh (full screen)
        style={{
      ...
      >
        ...
        <div className="absolute inset-0 bg-black bg-opacity-50 flex items-start justify-start">
          <div className="w-full max-w-lg ml-10 mt-10"> {/* Added margin for positioning */}
            <FindCars />
          </div>
        </div>

        {/* Carousel controls with text links positioned at the bottom of the background image */}
        <div className="absolute bottom-0 w-full flex justify-center space-x-8 mb-4">
          <button
            className={`text-lg ${activeImage === carBg ? 'font-bold' : 'font-normal'}`}
            onClick={() => setActiveImage(images.carBg)}
          >
            Cars
          </button>
          <button
            className={`text-lg ${activeImage === citiesBg ? 'font-bold' : 'font-normal'}`}
            onClick={() => setActiveImage(images.citiesBg)}
          >
            Cities
          </button>
          <button
            className={`text-lg ${activeImage === logoImage ? 'font-bold' : 'font-normal'}`}
            onClick={() => setActiveImage(images.logoImage)}
          >
            Logos
          </button>
          <button
            className={`text-lg ${activeImage === serviceBg ? 'font-bold' : 'font-normal'}`}
